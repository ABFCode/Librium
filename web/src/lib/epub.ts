...
function flattenTOC(items: TOCItem[], out: SectionInfo[], depth: number, parent?: number): void {
...
function convertInlines(inlines: Inline[], baseHref: string, book: Book, images: Map<string, ImagePayload>): Inline[] {
  return inlines.map((inline) => {
    ...
    return { ...inline, src: resolved, width: meta?.width, height: meta?.height }
  })
}

function convertBlock(block: Block, baseHref: string, book: Book, images: Map<string, ImagePayload>): Block {
  const out: Block = { ...block }
  if (block.inlines) out.inlines = convertInlines(block.inlines, baseHref, book, images)
  if (block.table) {
    out.table = {
      rows: block.table.rows.map((r) => ({
        cells: r.cells.map((c) => ({ ...c, inlines: convertInlines(c.inlines, baseHref, book, images) })),
      })),
    }
  }
  if (block.figure) {
...
function ensureImage(book: Book, href: string, images: Map<string, ImagePayload>): void {
...
function buildSectionBlocks(book: Book, sections: SectionInfo[]): { sectionBlocks: SectionBlocksPayload[]; images: ImagePayload[] } {
  ...
  interface Target { sectionIndex: number; spineIndex: number; blockIndex: number; baseHref: string }
      ...
      if (ref) {
        const baseHref =
          ref.spineIndex >= 0 && ref.spineIndex < book.spine.length
            ...
            : ''
        targets.push({ sectionIndex: section.orderIndex, spineIndex: ref.spineIndex, blockIndex: ref.blockIndex, baseHref })
        continue
      }
    }
    if (section.href) {
      ...
      if (idx !== undefined) {
        targets.push({ sectionIndex: section.orderIndex, spineIndex: idx, blockIndex: 0, baseHref: normalizeHrefPath(section.href) })
      }
    }
  }
  ...
  for (const [spineIndex, list] of bySpine) {
    ...
    list.sort((a, b) => (a.blockIndex === b.blockIndex ? a.sectionIndex - b.sectionIndex : a.blockIndex - b.blockIndex))
    for (let i = 0; i < list.length; i++) {
      ...
      const payloadBlocks = slice.map((b) => convertBlock(b, target.baseHref, book, images))
      if (payloadBlocks.length > 0) sectionBlocksMap.set(target.sectionIndex, payloadBlocks)
    }
  }
  ...
  for (const section of sections) {
    ...
    if (blocks) sectionBlocks.push({ sectionOrderIndex: section.orderIndex, blocks })
  }
  const imageList = [...images.values()].sort((a, b) => (a.href < b.href ? -1 : a.href > b.href ? 1 : 0))
...
function buildChunkPayloads(book: Book, sections: SectionInfo[]): ChunkPayload[] {
  const chunks = book.chunks(CHUNKING)
  const secs = sections.length > 0 ? sections : [{ orderIndex: 0, anchorHref: '' } as SectionInfo]
  ...
  if (anchors.length === 0 && secs.length > 0) anchors.push({ sectionIndex: 0, chunkIndex: 0 })
...
export function parseEpubToPayload(bytes: Uint8Array): EpubPayload {
  ...
  return {
    ...
    sectionBlocks,
    images,
    cover: cover ? { contentType: cover.contentType, bytes: cover.bytes } : undefined,
  }
}
