...
test('sign up, import, read, restore, search', async ({ page }) => {
  ...
  await result.click()
  await expect(page.locator('.reader-topbar-title')).toHaveText(
    /Chapter II/,
    { timeout: 15_000 },
  )
  await expect
    .poll(async () =>
      scroller(page).evaluate((el) => {
        const match = [...el.querySelectorAll('[data-chunk-index]')].find(
          (b) => b.textContent?.includes('xylophone-harvest'),
